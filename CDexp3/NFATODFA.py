...
for i in range(n):
    ...
    for j in range(t):
        path = input("symbol : ")  # Enter path eg : a or b in {a,b} 0 or 1 in {0,1}
        print("Enter end state from state {} for the input symbol {} : ".format(state, path))
...
x = len(new_states_list)
while x != 0:  # consition is true only if the new_states_list is not empty
